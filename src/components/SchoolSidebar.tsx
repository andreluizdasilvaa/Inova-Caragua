...
export const SchoolSidebar: React.FC<SchoolSidebarProps> = ({ 
  currentView, 
  setView, 
  session, 
  onLogout 
  ...
  return (
    <aside className="w-60 bg-slate-900 border-r border-slate-800 text-slate-100 flex flex-col h-screen fixed left-0 top-0 z-20 shrink-0 font-sans">
      ...
      <div className="p-4 border-b border-slate-800">
        <Image 
          src={Logo}
          alt='Logo Inova Caragua - Escola'
        />
      </div>

      {/* Main Navigation links */}
      <nav className="flex-1 p-3 space-y-1 overflow-y-auto">
        <p className="px-2 text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Escola</p>
