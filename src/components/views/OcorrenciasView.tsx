  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h2 className="text-lg font-extrabold text-slate-900 tracking-tight">Gestão de Ocorrências</h2>
          <p className="text-xs text-slate-400">Monitore, filtre e direcione chamados de infraestrutura em tempo real.</p>
        </div>
        <Button 
          variant="secondary" 
          onClick={() => setView('nova-ocorrencia')}
          className="bg-brand-blue hover:bg-brand-teal text-xs py-1.5 px-3"
        >
      ...
      <Card className="p-3.5">
        <div className="flex items-center gap-2 text-xs font-bold text-slate-700 mb-3.5 pb-1.5 border-b border-slate-100">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
          ...
          <div className="space-y-1">
            ...
            <select
              ...
              className="w-full text-xs rounded border border-slate-200 bg-slate-50 py-1.5 px-2 outline-none focus:ring-1 focus:ring-brand-blue transition-all text-slate-700 font-semibold"
            >
          ...
          {/* School dropdown */}
          <div className="space-y-1">
            <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Localização / Unidade</label>
            <select
        ...
        <div className="mt-4 pt-3 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-3">
          <div className="relative w-full sm:w-64">
            ...
            <input
              ...
              className="w-full text-xs rounded border border-slate-200 bg-slate-50 pl-8 pr-3 py-1.5 focus:bg-white focus:outline-none focus:ring-1 focus:ring-brand-blue transition-all placeholder:text-slate-400"
            />
          </div>

          <div className="flex gap-2 w-full sm:w-auto shrink-0 justify-end">
            <Button variant="outline" size="sm" onClick={handleClearFilters} className="text-[11px] py-1 px-2.5">
              Limpar Filtros
            </Button>
            <Button variant="primary" size="sm" onClick={() => {}} className="bg-slate-800 hover:bg-slate-900 text-[11px] py-1 px-2.5">
              Aplicar Filtros
            </Button>
          </div>
        </div>
      ...
      <Card className="overflow-hidden">
          ...
          <table className="w-full text-left border-collapse font-sans">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200 text-[10px] font-bold text-slate-500 uppercase tracking-wider">
                ...
                <th className="py-2.5 px-4">Escola</th>
            ...
            <tbody className="divide-y divide-slate-100 text-xs font-semibold text-slate-600">
              {filteredOccurrences.length > 0 ? (
                filteredOccurrences.map((occ) => (
                  ...
                  >
                    <td className="py-2 px-4 font-mono text-[11px] font-bold text-slate-500">{occ.id}</td>
                    <td className="py-2 px-4 font-bold text-slate-900">{occ.school}</td>
                    <td className="py-2 px-4 text-slate-500">{occ.category}</td>
                    ...
                    <td className="py-2 px-4 text-slate-500">{occ.date}</td>
                    <td className="py-2 px-4 text-right space-x-1 whitespace-nowrap">
                      <button
                        ...
                        className="p-1 text-brand-blue hover:text-white hover:bg-brand-blue rounded transition-all duration-150 inline-flex items-center cursor-pointer"
                      >
                      ...
                      <button
                        ...
                        className="p-1 text-slate-400 hover:text-slate-700 hover:bg-slate-100 rounded transition-all duration-150 inline-flex items-center cursor-pointer"
                      >
        ...
        <div className="bg-slate-50 border-t border-slate-200 px-4 py-2.5 flex items-center justify-between text-[10px] font-bold text-slate-500">
          <span>
            Mostrando 1 a {filteredOccurrences.length} de {filteredOccurrences.length} entradas
          </span>
          <div className="flex gap-1">
            <button disabled className="px-2 py-1 rounded border border-slate-200 bg-white hover:bg-slate-50 transition-colors disabled:opacity-40">
              Anterior
            </button>
            <button className="px-2 py-1 rounded border border-brand-blue bg-brand-blue text-white font-bold">
              1
            </button>
            <button disabled className="px-2 py-1 rounded border border-slate-200 bg-white hover:bg-slate-50 transition-colors disabled:opacity-40">
              Próximo
