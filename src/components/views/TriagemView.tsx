  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h2 className="text-lg font-extrabold text-slate-900 tracking-tight">Central de Triagem</h2>
          <p className="text-xs text-slate-400">Avalie, priorize e encaminhe relatórios de infraestrutura para equipes técnicas.</p>
        </div>
      </div>

      {/* Main Toast Notifications */}
      {showToast && (
        <div className="fixed top-16 right-4 bg-slate-900 text-white border border-slate-700 px-3.5 py-2 rounded shadow-lg z-50 flex items-center gap-2.5 animate-fade-in">
      ...
      {/* Two Pane Workspace */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 items-start h-[calc(100vh-8.5rem)] min-h-[460px]">
        ...
        <div className="lg:col-span-4 bg-white border border-slate-200 rounded flex flex-col h-full overflow-hidden">
          <div className="py-2 px-3 border-b border-slate-150 bg-slate-50 shrink-0 flex items-center justify-between">
            <h3 className="text-xs font-bold text-slate-800 flex items-center gap-2">
              <FileText className="w-3.5 h-3.5 text-brand-blue" />
              Fila de Triagem
            </h3>
            <span className="font-mono text-[9px] font-bold text-slate-500 bg-slate-200 px-1.5 py-0.5 rounded">
            ...
            {occurrences.map((occ) => {
              ...
              return (
                <div
                  ...
                  className={`border rounded p-2.5 cursor-pointer transition-all duration-150 relative ${
                    isSelected
                ...
                >
                  ...
                  <div className="flex justify-between items-center mb-1 pr-3">
                    ...
                    <span className="text-[9px] text-slate-400 font-medium">{occ.date}</span>
                  </div>
                  ...
                  <div className="flex items-center justify-between mt-2 pt-1.5 border-t border-slate-100">
                    ...
                    <span className="text-[9px] font-mono font-medium text-slate-400">{occ.category}</span>
                  </div>
                </div>
              );
            })}
        ...
        <div className="lg:col-span-8 flex flex-col h-full gap-3 overflow-y-auto pr-1">
          {currentOcc ? (
            <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3.5">
                ...
                <Card className="p-3.5 flex flex-col justify-between space-y-3">
                  <div>
                    <div className="flex items-center gap-1.5 mb-1.5">
                      <span className="font-mono text-[9px] font-bold text-slate-500 bg-slate-100 px-1.5 py-0.5 rounded border border-slate-200">
                        {currentOcc.id}
                      </span>
                  ...
                  <div className="bg-slate-50 border border-slate-200 rounded p-2 text-xs text-slate-700 font-medium leading-relaxed max-h-[100px] overflow-y-auto">
                    &ldquo;{currentOcc.description}&rdquo;
                  </div>

                  <div className="grid grid-cols-2 gap-3 pt-1.5 border-t border-slate-100 shrink-0">
                    <div>
                      <span className="text-[9px] font-bold text-slate-400 uppercase tracking-wider block mb-0.5">Categoria</span>
                    ...
                    <div>
                      <span className="text-[9px] font-bold text-slate-400 uppercase tracking-wider block mb-0.5">Anexos</span>
                      {currentOcc.attachments && currentOcc.attachments.length > 0 ? (
                        <div className="flex gap-1">
                          {currentOcc.attachments.map((file, i) => (
                            <span key={i} className="inline-flex items-center gap-0.5 px-1.5 py-0.5 text-[9px] font-semibold rounded bg-slate-100 text-slate-700 border border-slate-200 hover:bg-slate-200 transition-colors cursor-pointer">
                        ...
                        <span className="text-[10px] text-slate-400 font-medium">Sem anexos</span>
                      )}
                    </div>
                  </div>
                </Card>
                ...
                <Card className="p-3.5 flex flex-col justify-between">
                  <div className="flex items-center gap-1.5 pb-1.5 border-b border-slate-100 mb-3 shrink-0">
                    <School className="w-3.5 h-3.5 text-brand-blue" />
                    <h3 className="text-xs font-bold text-slate-800">Contexto Escolar</h3>
                  </div>
                  ...
                  <div className="space-y-3 flex-1 flex flex-col justify-center">
                    <div className="flex items-center justify-between p-2.5 bg-slate-50 border border-slate-200 rounded">
                      ...
                      <span className="text-xs font-bold text-brand-blue">{schoolStats.school}</span>
                    </div>

                    <div className="grid grid-cols-2 gap-3">
                      <div className="p-2.5 bg-slate-100 rounded border border-slate-200 text-center flex flex-col justify-center">
                        <span className="text-lg font-black text-slate-800 leading-none">{schoolStats.openCount}</span>
                        <span className="text-[9px] font-bold text-slate-500 uppercase tracking-wider mt-1">Chamados Abertos</span>
                      </div>

                      <div className="p-2.5 bg-rose-50 rounded border border-rose-200 text-center flex flex-col justify-center">
                        <span className="text-lg font-black text-rose-600 leading-none">{schoolStats.similarCases30d}</span>
                        <span className="text-[9px] font-bold text-rose-500 uppercase tracking-wider mt-1">Casos Similares</span>
                      </div>
                    </div>
                  </div>
                </Card>
              ...
              {currentOcc.assetPatrimony && (
                <Card className="overflow-hidden shrink-0">
                  <div className="py-2 px-3 bg-slate-50 border-b border-slate-200 flex items-center justify-between flex-wrap gap-2">
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-left text-xs border-collapse font-sans">
                      <thead>
                        <tr className="bg-slate-50 border-b border-slate-150 text-[9px] font-bold text-slate-500 uppercase tracking-wider">
                          <th className="py-1.5 px-3">Data</th>
                          <th className="py-1.5 px-3">Ocorrência</th>
                          <th className="py-1.5 px-3">Resolução</th>
                          <th className="py-1.5 px-3">Técnico</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-slate-100 text-[11px] text-slate-600 font-medium">
                        {assetHistoryList.map((hist, i) => (
                          <tr key={i} className="hover:bg-slate-50/50 transition-colors">
                            <td className="py-1.5 px-3 text-slate-800">{hist.date}</td>
                            <td className="py-1.5 px-3 font-semibold text-slate-900">{hist.occurrence}</td>
                            <td className="py-1.5 px-3 text-slate-500 max-w-[280px] truncate" title={hist.resolution}>
                              {hist.resolution}
                            </td>
                            <td className="py-1.5 px-3 text-slate-700">{hist.technician}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
              ...
              <Card className="p-3.5 flex flex-col justify-between shrink-0 relative">
                <div className="flex items-center gap-1.5 pb-1.5 border-b border-slate-100 mb-3.5 shrink-0">
                  <Gavel className="w-3.5 h-3.5 text-brand-blue" />
                  <h3 className="text-xs font-bold text-slate-800">Ação de Triagem de Chamado</h3>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-3.5">
                      ...
                      {['Baixa', 'Média', 'Alta'].map((p) => {
                        ...
                        return (
                          <button
                            ...
                            className={`border rounded py-1.5 text-center text-[10px] font-bold transition-all cursor-pointer ${colors[p as 'Baixa'|'Média'|'Alta']}`}
                          >
                            {p}
                          </button>
                        );
                  ...
                  <div className="space-y-2">
                    ...
                    <select
                      ...
                      className="w-full text-xs rounded border border-slate-200 bg-white py-1.5 px-2 outline-none focus:ring-1 focus:ring-brand-blue transition-all font-semibold text-slate-700"
                    >
                ...
                <div className="space-y-1.5 mb-3.5">
                  ...
                  <textarea
                    ...
                    className="w-full text-xs border border-slate-200 rounded p-2 outline-none focus:ring-1 focus:ring-brand-blue transition-all bg-white font-medium placeholder:text-slate-400 resize-none"
                  />
                </div>
                ...
                <div className="flex flex-col sm:flex-row gap-2 justify-end pt-3 border-t border-slate-100">
                  <Button
                    ...
                    className="border-slate-300 text-slate-600 hover:bg-slate-50 text-[10px] py-1 px-2.5"
                  >
                  ...
                  <Button
                    ...
                    className="bg-brand-blue hover:bg-brand-teal text-[10px] py-1 px-2.5"
                  >
            ...
            <div className="flex flex-col items-center justify-center h-full text-slate-400 p-8 text-center border-2 border-dashed border-slate-200 rounded-xl">
              ...
              <p className="text-sm font-semibold">Nenhum chamado de triagem selecionado.</p>
              <p className="text-xs">Selecione uma ocorrência na fila lateral para analisar os dados.</p>
            </div>
          )}
        </div>
